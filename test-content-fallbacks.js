...
function testContentConversion(doc, testName) {
	...
	console.log(
		`Content preview: "${markdown.substring(0, 100)}${markdown.length > 100 ? '...' : ''}"`
	);
...
const validProseMirrorDoc = {
	...
	notes: {
		...
		content: [
			{
				type: 'paragraph',
				content: [{ type: 'text', text: 'This is valid ProseMirror content.' }],
			},
		],
	},
...
const result2 = testContentConversion(
	invalidProseMirrorDoc,
	'Test 2: Invalid ProseMirror, Valid Markdown'
);
...
const result5 = testContentConversion(
	whitespaceDoc,
	'Test 5: Whitespace ProseMirror, Valid Markdown'
);
		...
		content: [
			{
				...
				content: [{ type: 'text', text: 'Weekly 1:1' }],
			},
			{
				type: 'paragraph',
				content: [{ type: 'text', text: 'Discussed project status and next steps.' }],
			},
		],
	},
...
results.forEach((result, index) => {
	...
	const hasContent =
		result.markdown.trim().length > 0 && !result.markdown.includes('No content available');
	console.log(
		`Test ${testNum}: ${hasContent ? '✅' : '❌'} ${result.contentSource} (${result.markdown.length} chars)`
	);
});

const successfulTests = results.filter(
	r => r.markdown.trim().length > 0 && !r.markdown.includes('No content available')
).length;
...
console.log(
	'- If ProseMirror validation is too strict, we should see more markdown/plain fallbacks'
);
console.log(
	'- If user reports empty content, it suggests API fields are all empty or fallbacks are broken'
);
