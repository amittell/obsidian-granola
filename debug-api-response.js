...
async function debugAPIResponse() {
	...
	try {
			...
			if (doc.notes) {
				...
				if (typeof doc.notes === 'object') {
					...
					if (doc.notes.content) {
						...
						console.log(
							`   - Sample structure:`,
							JSON.stringify(doc.notes, null, 2).substring(0, 200) + '...'
						);
					}
				}
			} else {
		...
		console.log(
			`Documents with notes (ProseMirror): ${docsWithNotes} (${((docsWithNotes / documents.length) * 100).toFixed(1)}%)`
		);
		console.log(
			`Documents with notes_plain: ${docsWithPlain} (${((docsWithPlain / documents.length) * 100).toFixed(1)}%)`
		);
		console.log(
			`Documents with notes_markdown: ${docsWithMarkdown} (${((docsWithMarkdown / documents.length) * 100).toFixed(1)}%)`
		);
		console.log(
			`Documents with ANY content: ${docsWithAnyContent} (${((docsWithAnyContent / documents.length) * 100).toFixed(1)}%)`
		);
