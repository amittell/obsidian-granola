...
export class DocumentSelectionModal extends Modal {
	...
	private renderDocumentItem(container: HTMLElement, doc: DocumentDisplayMetadata): void {
		...
		// Initial progress is 0% (set via CSS custom property)
	...
	private updateDocumentProgress(docProgress: DocumentProgress): void {
		...
		// Update progress bar
		progressFill.style.setProperty('--progress-width', `${docProgress.progress}%`);
	...
	private applyStyles(): void {
		...
		// The styles are now in src/styles/document-selection-modal.css
		// In Obsidian plugins, CSS files in the styles folder are automatically loaded
		// But we also add the classes that were already being added dynamically
	}
	...
	private cleanup(): void {
		// Cleanup is handled by onClose method
		// Styles are now in external CSS file (styles.css)
	}
